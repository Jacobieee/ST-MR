...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        all_f = [os.path.splitext(os.path.basename(f))[0] for f in all_files]
        all_f = np.array(all_f)
        # np.save('files.npy', all_f)
        ...
        for path in all_files:
            ...
            for idx in range(0, 1):
                ...
                for i, ped_id in enumerate(peds_in_curr_seq):
                    ...
                    if float(p_id) == ped_id:
                      p_temp = num_peds_considered
        ...
        all_f = np.delete(all_f, del_lst)
        np.save('files.npy', all_f)

        assert(pid.shape == (478,))
        assert(all_f.shape == (478,))
        ...
        self.pid = torch.from_numpy(pid).type(torch.float)
